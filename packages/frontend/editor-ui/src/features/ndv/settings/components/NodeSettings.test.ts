...
describe('NodeSettings', () => {
	...
	describe('restricted node type', () => {
		...
		it('shows the callout and makes the parameters read-only', async () => {
			const { findByTestId, getAllByTestId } = renderNodeSettings({
				...restricted,
				stubs: {
					ParameterInputList: {
						props: ['isReadOnly'],
						template: '<div data-test-id="parameter-input-list" :data-read-only="isReadOnly" />',
					},
				},
			});

			expect(await findByTestId('node-restricted-callout')).toHaveTextContent(
